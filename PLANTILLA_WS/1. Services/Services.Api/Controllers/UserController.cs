...
namespace Services.Api.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public IActionResult UpdateUser(UserViewModel userViewModel)
        {
            ...
            try
            {
                var userResult = _usersAppService.UpdateUser(userViewModel);
                if (userResult == null)
                {
                    response.statusCode = 200;
                    response.message = "No existe el usuario";
                ...
                response.statusCode = 200;
                response.message = "Usuario actuaizado con exito";
                response.ok = true;
                response.data = userResult;
                return Ok(response);
            }
            ...
            {
                _logger.LogError(ex, "Ha ocurrido un error al actualizar el usuario");
                response.statusCode = 404;
                response.message = "Ha ocurrido un error al actualizar el usuario";
                response.ok = false;
                return StatusCode(StatusCodes.Status404NotFound, response);
            }
        }
